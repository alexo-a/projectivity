...
export function createProjectReportPDF(compiledInfo, title){
    //this function uses the compiled data from the Project Report and makes the 
    //guts of a pdf (using pdfmake) that we then open in a new tab.
    function generateBody(taskArray){
        ...
        taskArray.map(task => {
            let subTableBody = task.users.map(user => {
                return [
                    { text: user.username, alignment: "center" },
                    { text: user.duration, alignment: "center" }
                ]
            })
            subTableBody.push(["", { text: task.totalTime, alignment: "center", border: [false, true, false, false] }])
            results.push(
                [
                    { text: "", border: [false, false, false, false] },
                    task.taskTitle,
